...
namespace
{
    ...
    void printUsage()
    {
        std::cout
            ...
            << "    Alembic and USD are selected by .abc/.usd/.usda/.usdc output extension.\n"
            << "    Some optional backends may be unavailable; run 'houio capabilities'.\n"
            << "  --no-overwrite\n"
    ...
    int inspectFile(const std::filesystem::path &path, bool json)
    {
        ...
        std::size_t packedDiskSequenceRecords = 0;
        ...
        for( const houio::HouGeoAdapter::Primitive::Ptr &primitive : result.value->primitives() )
        {
            if( std::dynamic_pointer_cast<houio::HouGeoAdapter::PolyPrimitive>(primitive) )
                ...
                ++packedDiskRecords;
            else if( std::dynamic_pointer_cast<houio::HouGeoAdapter::PackedDiskSequencePrimitive>(primitive) )
                ++packedDiskSequenceRecords;
        ...
        if( json )
        {
            std::cout << "{\"success\":true,\"path\":\"" << jsonEscape(path.string())
                ...
                << ",\"packed_disk_sequence_records\":" << packedDiskSequenceRecords
