...
- **Convert Geometry File** runs `houio convert` for `.geo`, `.bgeo`, and `.bgeo.sc` files.
- **Package Diagnostics** reports the active package, HouIO executable, and C-Blosc paths.

The HouIO CLI runs as a separate process. Houdini loads only the Python bridge, avoiding Python ABI coupling between Houdini versions.
