...
namespace houio
{
	...
	void HouGeo::addPrimitive( SparseVec3fVdbPrimitive::Ptr sparseVdb )
	{
		if( !sparseVdb )
			throw std::invalid_argument( "HouGeo::addPrimitive received a null sparse Vec3f VDB" );
		m_primitives.push_back(std::move(sparseVdb));
	}
					...
					for( sint64 elementIndex=0;elementIndex<elementCount;++elementIndex )
					{
						...
						for( int componentIndex=0;componentIndex<attrTupleSize;++componentIndex )
						{
							...
							hougeo_attribute_detail::storeNumericComponent(
								data, destinationIndex, attrStorage, tuple->value(componentIndex));
						}
					}
				}
					...
					for( int componentIndex=0;componentIndex<attrTupleSize;++componentIndex )
					{
						...
						for( sint64 elementIndex=0;elementIndex<elementCount;++elementIndex )
						{
							const size_t destinationIndex = static_cast<size_t>(elementIndex)
								* static_cast<size_t>(attrTupleSize) + static_cast<size_t>(componentIndex);
							hougeo_attribute_detail::storeNumericComponent(
								data, destinationIndex, attrStorage,
						...
						for( std::vector<ubyte>::iterator it = attrPacking.begin(); it != attrPacking.end();++it, ++packIndex )
						{
							...
							for( size_t i=0;i<numElements;++i )
							{
								...
								for( size_t component=0;component<maxPack;++component )
								{
									...
									hougeo_attribute_detail::storeNumericComponent(
										data, destElementIndex + startComponentIndex + component,
	...
	int HouGeo::HouSparseVec3fVdb::topologyVertex() const
	{
		return topology_vertex_;
	}

	const SparseVec3fGrid& HouGeo::HouSparseVec3fVdb::sparseGrid() const
	{
		return sparse_grid_;
	}
