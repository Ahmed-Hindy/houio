...
Embedded `PackedGeometry`, named `PackedFragment`, external `PackedDisk`, `PackedDiskSequence`, and opaque native VDB payload preservation are supported. Dependency-neutral sparse FloatGrid and Int32Grid editing is available through the C++ API. An opt-in OpenVDB build adds standalone `.vdb` I/O through `OpenVdbBackend`; the CLI can use that backend to construct native Houdini VDB records from `sparse_float_vdb` or `sparse_int32_vdb` manifests inside GEO/BGEO/SCF output, including bounded `active_tiles` without forced densification, but does not provide standalone `.vdb` output. `houio diagnose` reports whether that backend was compiled and its version. `houio inspect` reports separate counts for packed geometry, packed fragments, packed disks, packed disk sequences, and native VDB records.
