...
NURBS and Bezier curve records, native Sphere and Tube records, embedded `PackedGeometry`, named `PackedFragment`, external `PackedDisk`, `PackedDiskSequence`, and opaque native VDB payload preservation are supported. Explicit manifests use `nurbs_curve` or `bezier_curve`; direct HOM extraction supports exact Bezier records, while NURBS requires file-level preservation or an explicit manifest because HOM does not expose endpoint-interpolation policy. Dependency-neutral sparse FloatGrid, Int32Grid, and Vec3fGrid editing is available through the C++ API. An opt-in OpenVDB build adds standalone `.vdb` I/O through `OpenVdbBackend`; the CLI can use that backend to construct native Houdini VDB records from `sparse_float_vdb`, `sparse_int32_vdb`, or `sparse_vec3f_vdb` manifests inside GEO/BGEO/SCF output, including bounded `active_tiles` without forced densification, but does not provide standalone `.vdb` output. Vec3f manifests also preserve explicit vector transformation semantics and staggered-grid class. `houio diagnose` reports whether that backend was compiled and its version. `houio inspect` reports separate counts for polygon, curve, sphere, tube, packed geometry, packed fragment, packed disk, packed disk sequence, dense-volume, and native VDB records.
