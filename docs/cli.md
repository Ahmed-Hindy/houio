...
Embedded `PackedGeometry`, named `PackedFragment`, external `PackedDisk`, `PackedDiskSequence`, and opaque native VDB payload preservation are supported. OpenVDB-backed sparse-tree construction/editing remains a separate optional addition. `houio inspect` reports separate counts for packed geometry, packed fragments, packed disks, packed disk sequences, and native VDB records.
