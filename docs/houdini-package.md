...
The package supports HouIO's polygon, embedded `PackedGeometry`, named `PackedFragment`, external `PackedDisk`, `PackedDiskSequence`, numeric/string/dictionary attribute, group, and dense scalar-volume model. Packed-disk authored filenames, expansion frames/policies, transforms, viewport LOD, and flags are preserved without opening or copying the referenced file. Houdini Volume Visualization detail metadata is preserved in both the scalar-attribute layout used by Houdini 20.x and the dictionary layout used by Houdini 21.x and newer.
