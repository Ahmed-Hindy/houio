...
Existing native VDB primitive payloads are preserved losslessly by file-to-file HouIO round trips. The standalone C++ API now includes dependency-neutral sparse FloatGrid editing, and an opt-in OpenVDB build can read and write native `.vdb` FloatGrids. Direct live-HOM writing of native VDB primitives is still unavailable because HOM does not expose the serialized payload and HouIO has not yet converted the sparse-grid model into that Houdini record. The compatibility round-trip path can still densify supported Float SDF/Fog grids and restore their class.
